...
// column -1, then -2, forever (see TransformationAlgorithmCluster's own
// class comment). So the base class constructor call below passes
// `allowed`, which blocks every anchor below column 0 -- every bit that
// starts above column 0 still gets split all the way down to it, and a
// bit already at column 0 is never touched, since producing it would
// require an anchor at column -1, which is disallowed.
//
// Like MergeCluster (merge_cluster.hpp), a BinaryFormCluster *is* a
// TransformationAlgorithmCluster configured a particular way -- just
// SplitTransformation, named "binary_form", bounded to column >= 0 -- so
// name()/run() are simply inherited. See MergeCluster's own class comment
// for why splitTransformation() below is a function-local static rather
// than an instance member: a plain member's address wouldn't be safe to
// hand to the base constructor, since base classes finish constructing
// before any of a derived class's own members even begin.
class BinaryFormCluster : public TransformationAlgorithmCluster {
public:
    BinaryFormCluster()
        : TransformationAlgorithmCluster({&splitTransformation()}, "binary_form",
                                          [](int, int j) { return j >= 0; }) {}
